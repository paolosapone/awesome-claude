...
export function getUiColors(context: ThemeContext): WorkbenchColors {
    ...
    // Workbench modes
    const workbenchBase =
        options.workbenchMode === "minimal"
            ? palette.base
            : options.workbenchMode === "flat"
            ? palette.mantle
            : palette.crust;

    const dropBackground = opacity(accent, 0.2);
    const border = isHighContrast
        ? opacity(palette.overlay0, 0.2)
        : transparent;

    // Bracket colors for rainbow mode
    const bracketColors =
        options.bracketMode === "rainbow"
            ? [
                  palette.red,
                  palette.orange,
                  palette.yellow,
    ...
    return {
        ...
        "statusBarItem.activeBackground": isDark
            ? opacity(palette.text, 0.2)
            : opacity(palette.white, 0.2),
        "statusBarItem.hoverBackground": isDark
            ? opacity(palette.text, 0.1)
            : opacity(palette.white, 0.1),
        "statusBarItem.prominentForeground": isDark
            ? palette.text
            : palette.white,
        ...
        "statusBarItem.warningForeground": isDark
            ? palette.text
            : palette.crust,
        ...
        "gitDecoration.deletedResourceForeground": isDark
            ? "#F87171"
            : "#DC2626",
        "gitDecoration.renamedResourceForeground": isDark
            ? "#60A5FA"
            : "#2563EB",
        "gitDecoration.stageModifiedResourceForeground": isDark
            ? "#FDE047"
            : "#FACC15",
        "gitDecoration.stageDeletedResourceForeground": isDark
            ? "#FCA5A5"
            : "#F87171",
        "gitDecoration.untrackedResourceForeground": isDark
            ? "#4DD4E8"
            : "#0891B2",
        "gitDecoration.ignoredResourceForeground": isDark
            ? "#9CA3AF"
            : "#6B7280",
        "gitDecoration.conflictingResourceForeground": isDark
            ? "#FB923C"
            : "#EA580C",
        "gitDecoration.submoduleResourceForeground": isDark
            ? "#C084FC"
            : "#9333EA",
    };
}
